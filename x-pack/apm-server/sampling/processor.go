...
func (p *Processor) ProcessBatch(ctx context.Context, batch *modelpb.Batch) error {
	...
	for i := 0; i < len(events); i++ {
		...
		var err error
		if event.Processor.Name == "transaction" && event.Processor.Event == "transaction" {
			atomic.AddInt64(&p.eventMetrics.processed, 1)
			report, stored, err = p.processTransaction(event)
		} else if event.Processor.Name == "transaction" && event.Processor.Event == "span" {
			atomic.AddInt64(&p.eventMetrics.processed, 1)
			...
			if n := len(events); n > 0 {
				...
				if remoteDecision {
					...
					for _, event := range events {
						...
						} else if event.Processor.IsSpan() {
