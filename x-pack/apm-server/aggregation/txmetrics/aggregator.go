...
type AggregatorConfig struct {
	// BatchProcessor is a modelpb.BatchProcessor for asynchronously processing metrics documents.
...
func makeMetricset(key transactionAggregationKey, metrics transactionMetrics, interval string) *modelpb.APMEvent {
	...
	return &modelpb.APMEvent{
		...
		Kubernetes: &modelpb.Kubernetes{PodName: key.kubernetesPodName},
		Service: &modelpb.Service{
			Name:        key.serviceName,
			Version:     key.serviceVersion,
			Node:        &modelpb.ServiceNode{Name: key.serviceNodeName},
...
// And docs/data-model.asciidoc for the current documentation on the APM Server modelpb.
