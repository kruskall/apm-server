	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Empty(t, cmp.Diff(
				expectedOverflowMetricsets,
				metricsets,
				cmpopts.IgnoreSliceElements(func(a *modelpb.APMEvent) bool {
					return a.Transaction.Name != "_other"
				}),
				cmpopts.IgnoreTypes(netip.Addr{}),
				cmpopts.IgnoreFields(modelpb.APMEvent{}, "Timestamp", "Service.Name"),
...
func TestAggregatorRun(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		for _, event := range metricsets {
			assert.Equal(t, now.AsTime().Truncate(intervals[i]), event.Timestamp.AsTime())
...
func TestAggregationFields(t *testing.T) {
	...
	batch := expectBatch(t, batches)
	metricsets := batchMetricsets(t, batch)
	require.Empty(t, cmp.Diff(expected, metricsets,
		cmp.FilterPath(func(p cmp.Path) bool {
			switch p.Last().String() {
			case ".state", ".sizeCache", ".unknownFields":
				return true
