	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, interval := range append([]time.Duration{config.Interval}, config.RollUpIntervals...) {
				expectedEvents := tt.getExpectedEvents(now.AsTime(), interval)
...
func makeSpan(
	...
	event := modelpb.APMEvent{
		...
		Labels: map[string]*modelpb.LabelValue{
			"department_name": {Global: true, Value: "apm"},
			"organization":    {Global: true, Value: "observability"},
			"company":         {Global: true, Value: "elastic"},
		},
		NumericLabels: map[string]*modelpb.NumericLabelValue{
			"user_id":     {Global: true, Value: 100},
			"cost_center": {Global: true, Value: 10},
		},
	}
