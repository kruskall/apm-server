...
func rumRequestMetadataFunc(cfg *config.Config) func(c *request.Context) *modelpb.APMEvent {
	...
	return func(c *request.Context) *modelpb.APMEvent {
		e := modelpb.APMEvent{
			Client:    &modelpb.Client{Ip: c.ClientIP.String()},
			Source:    &modelpb.Source{Ip: c.SourceIP.String(), Port: uint32(c.SourcePort)},
			Timestamp: timestamppb.New(c.Timestamp),
			UserAgent: &modelpb.UserAgent{Original: c.UserAgent},
		}
